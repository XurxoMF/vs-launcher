...
function ListMods(): JSX.Element {
  ...
  const [installedModsVersions, setInstalledModsVersions] = useState<{ modid: string; versions: DownloadableModVersion[] | undefined }[]>([])
  ...
  async function queryAndSaveModVersions(): Promise<void> {
    const imv = await Promise.all(
      installedMods.map(async (im) => {
        try {
          const versions = await queryModVersions(im.modid)
          return { modid: im.modid, versions }
        } catch (err) {
          window.api.utils.logMessage("error", `[component] [ManageMods(installations)] Error fetching mod versions: ${err}`)
          return { modid: im.modid, versions: undefined }
        }
      })
    )
    setInstalledModsVersions(imv)
  }
              ...
              <div className="flex gap-4 p-2 justify-between items-center whitespace-nowrap">
                ...
                <div className="flex gap-2 justify-end text-lg">
                  <Button
                    ...
                    onClick={async () => {
                      const imv = installedModsVersions.find((imv) => imv.modid === iMod.modid)
                      if (imv === undefined) return addNotification(t("notifications.titles.error"), t("features.mods.noModFound"), "error")
                      if (!imv.versions || imv.versions.length < 1) return addNotification(t("notifications.titles.error"), t("features.mods.noVersionsFound"), "error")
                      setModToUpdate(iMod)
                    }}
                  >
                    U
            ...
            <DialogPanel className="w-[800px] flex flex-col gap-2 text-center bg-zinc-850 rounded p-8">
              ...
              <TableWrapper className="max-h-[300px] overflow-y-scroll text-center">
                ...
                <TableBody className="overflow-x-hidden">
                  {installedModsVersions
                    .find((imv) => imv.modid === modToUpdate.modid)!
                    .versions!.map((imvv) => (
                      <TableBodyRow key={imvv.releaseid} disabled={imvv.modversion === modToUpdate.version}>
                        <TableCell className="w-2/12">{imvv.modversion}</TableCell>
                        <TableCell className="w-2/12">{new Date(imvv.created).toLocaleDateString("es")}</TableCell>
                        <TableCell className="w-7/12 overflow-hidden whitespace-nowrap text-ellipsis">
                          <input type="text" value={imvv.tags.join(", ")} readOnly className="w-full bg-transparent outline-none text-center" />
                        </TableCell>
                        <TableCell className="w-1/12 flex gap-2 items-center justify-center">
                          <button
                            disabled={imvv.modversion === modToUpdate.version}
                            onClick={async (e) => {
                              e.preventDefault()
                              e.stopPropagation()
                              ...
                              startDownload(
                                t("features.mods.modTaskName", { name: modToUpdate.name, version: `v${imvv.modversion}`, installation: installation.name }),
                                t("features.mods.modDownloadDesc", { name: modToUpdate.name, version: `v${imvv.modversion}`, installation: installation.name }),
                                `https://mods.vintagestory.at/${imvv.mainfile}`,
                                installPath,
                                async (status, path, error) => {
                                  ...
                                  window.api.utils.logMessage("info", `[component] [ManageMods(installations)] Downloaded mod ${imvv.mainfile} on ${path}`)
                                  countMods()
                            ...
                            className={clsx(
                              ...
                              config.installations.some((i) => i.id === config.lastUsedInstallation) &&
                                imvv.tags.includes(`v${config.installations.find((i) => i.id === config.lastUsedInstallation)!.version}`)
                                ? "bg-green-700"
                                : imvv.tags.some((mvt) =>
                                      mvt.startsWith(
